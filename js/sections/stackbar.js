...
d3.csv("https://raw.githubusercontent.com/santiagoconde0/sabemosLoQueHiciste/master/data/modalidadcontratacion_1.csv").then(function(data) {
  console.log(data);
  color.domain(d3.keys(data[0]).filter(function(key) { return key !== "congresistafam"; }));
  ...
  x.domain(data.map(function(d) { return d.congresistafam; }));
  ...
  function change() {
    ...
    var x0 = x.domain(data.sort(sortDescending
        ...
        .map(function(d,i) { return d.congresistafam; }))
        .copy();
